...
时间记录仪是一款本地、隐私优先的 Windows 文件变化记录器。它默认实时监听本机所有可用固定磁盘，也可切换为自定义目录；记录创建、修改、重命名、移除和可关联的移动线索，并提供 SQLite 时间线、组合搜索、文件历史、完整性状态和本地导出。
...
- 默认只保留人为或软件路径中的变化；能够从程序目录识别时，来源会直接显示为“Codex（推定）”等软件名。其他事件会依据当时前台程序保守区分为“人为操作（推定）”或“软件操作（推定）”，Windows 系统后台路径默认不入库。
...
“人为操作”“软件操作”“Windows 系统”和“Microsoft Defender”来源按文件路径及当时前台程序推定，不等同于取得实际写入进程。“人为操作”仅在检测到资源管理器等文件管理器位于前台时使用；其余无法关联具体程序的事件归为“软件操作（推定）”。Defender 目录受系统权限保护时，相关记录属于尽力观察，不替代 Windows 安全中心中的“保护历史记录”。
